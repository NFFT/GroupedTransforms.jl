...
# Output:
 * `F::LinearMap{Float64}` ... Linear map of the Fourier-transform implemented by the NFFT
"""
function get_transform(bandwidths::Vector{Int}, X::Array{Float64}, dcos::Vector{String})::LinearMap
    if size(X, 1) == 1
        X = vec(X)
        d = 1
        M = length(X)
