...
function get_setting( system::String, d::Int, ds::Int, N::Vector{Int} )::Vector{NamedTuple{(:u, :mode, :bandwidths),Tuple{Vector{Int}, Module, Vector{Int}}}}
  ...
  return [ (u = U[idx], mode = systems[system], bandwidths = bandwidths[idx]) for idx in 1:length(U) ]
end

function get_setting( system::String, U::Vector{Vector{Int}}, N::Vector{Int} )::Vector{NamedTuple{(:u, :mode, :bandwidths),Tuple{Vector{Int}, Module, Vector{Int}}}}
  if !haskey(systems, system)
    error( "System not found." )
  end
  ...
  return [ (u = U[idx], mode = systems[system], bandwidths = bws[idx]) for idx in 1:length(U) ]
end
