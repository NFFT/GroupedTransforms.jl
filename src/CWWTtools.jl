...
function get_transform(bandwidths::Vector{Int}, X::Array{Float64}, m::Int)::Int64
    ...
    elseif d == 1    #1-dimensional terms
        ...
        for j = 1:bandwidths[1]
            num = min(2^j, 2 * m - 1)
            k =
                Int.(
                    mod.(
                        (floor.(2^j * X) .- 2 * m .+ 2) * ones(1, num) +
                        ones(M, 1) * transpose(collect((0:num-1))),
                        2^j,
                    ),
                )
    ...
    elseif d == 2
        ...
        for j in eachcol(freq)
            ...
            k1 =
                Int.(
                    mod.(
                        (floor.(2^j[1] * X[1, :]) .- 2 * m .+ 2) * ones(1, num1) +
                        ones(M, 1) * transpose(collect((0:num1-1))),
                        2^j[1],
                    ),
                )
            k2 =
                Int.(
                    mod.(
                        (floor.(2^j[2] * X[2, :]) .- 2 * m .+ 2) * ones(1, num2) +
                        ones(M, 1) * transpose(collect((0:num2-1))),
                        2^j[2],
                    ),
                )
            ...
            for kk1 = 1:num1
                ...
                k[:, (kk1-1)*num2+1:kk1*num2, :] =
                    cat(k1[:, kk1] * Int.(ones(num2)'), k2, dims = 3)
            end
    ...
    elseif d == 3 #3-dimensional terms
        ...
        for j in eachcol(freq)
            ...
            k3 =
                Int.(
                    mod.(
                        (floor.(2^j[3] * X[3, :]) .- 2 * m .+ 2) * ones(1, num3) +
                        ones(M, 1) * transpose(collect((0:num3-1))),
                        2^j[3],
                    ),
                )
            ...
            for kk1 = 1:num1
                for kk2 = 1:num2
                    k_ind[
                        :,
                        collect(
                            (kk1-1)*num2*num3+(kk2-1)*num3+1:((kk1-1)*num2*num3+(kk2)*num3),
                        ),
                    ] = 2^(j[2] + j[3]) * k1[:, kk1] + 2^(j[3]) * k2[:, kk2] .+ k3
                    k[
                        :,
                        collect(
                            (kk1-1)*num2*num3+(kk2-1)*num3+1:(kk1-1)*num2*num3+(kk2)*num3,
                        ...
                        k1[:, kk1] * Int.(ones(num3)'),
                        k2[:, kk2] * Int.(ones(num3)'),
                        k3,
                        dims = 3,
                    )
                end
            ...
            V = vcat(
                V,
                vec(reshape(transpose(Chui_periodic(X, m, [j[1], j[2], j[3]], k)), :, 1)),
            )
            ac_co = ac_co + 2^sum(j)
        end
        idx = length(trafos)
    ...
    elseif d == 4 #4-dimensional terms
        ...
        for j in eachcol(freq)
            ...
            k4 =
                Int.(
                    mod.(
                        (floor.(2^j[4] * X[4, :]) .- 2 * m .+ 2) * ones(1, num4) +
                        ones(M, 1) * transpose(collect((0:num4-1))),
                        2^j[4],
                    ),
                )
                ...
                for kk2 = 1:num2
                    for kk3 = 1:num3
                        k_ind[
                            :,
                            collect(
                                (kk1-1)*num2*num3*num4+(kk2-1)*num3*num4+(kk3-1)*num4+1:((kk1-1)*num2*num3*num4+(kk2-1)*num3*num4+(kk3)*num4),
                            ),
                        ] =
                            2^(j[2] + j[3] + j[4]) * k1[:, kk1] +
                            2^(j[3] + j[4]) * k2[:, kk2] +
                            2^(j[4]) * k3[:, kk3] .+ k4
                        k[
                            :,
                            ...
                            k1[:, kk1] * Int.(ones(num4)'),
                            k2[:, kk2] * Int.(ones(num4)'),
                            k3[:, kk3] * Int.(ones(num4)'),
                            k4,
                            dims = 3,
                        )
                    end
            ...
            V = vcat(
                ...
                vec(
                    reshape(
                        transpose(Chui_periodic(X, m, [j[1], j[2], j[3], j[4]], k)),
                        :,
                        1,
                    ),
                ),
    ...
    elseif d == 5 #5-dimensional terms
        ...
        J = Int.(ones(M))
        V = vec(
            Chui_periodic(X, m, [0; 0; 0; 0; 0], cat([0], [0], [0], [0], [0], dims = [3])),
        )
        freq = freq[:, 2:end]
        ac_co = 2                       # actual column index
        for j in eachcol(freq)
            ...
            k5 =
                Int.(
                    mod.(
                        (floor.(2^j[5] * X[5, :]) .- 2 * m .+ 2) * ones(1, num5) +
                        ones(M, 1) * transpose(collect((0:num5-1))),
                        2^j[5],
                    ),
                )
                    ...
                    for kk3 = 1:num3
                        for kk4 = 1:num4
                            k_ind[
                                :,
                                collect(
                                    (kk1-1)*num2*num3*num4*num5+(kk2-1)*num3*num4*num5+(kk3-1)*num4*num5+(kk4-1)*num5+1:((kk1-1)*num2*num3*num4*num5+(kk2-1)*num3*num4*num5+(kk3-1)*num4*num5+kk4*num5),
                                ),
                            ] =
                                2^(j[2] + j[3] + j[4] + j[5]) * k1[:, kk1] +
                                2^(j[3] + j[4] + j[5]) * k2[:, kk2] +
                                2^(j[4] + j[5]) * k3[:, kk3] +
                                2^(j[5]) * k4[:, kk4] .+ k5
                            k[
                                :,
                                ...
                                k1[:, kk1] * Int.(ones(num5)'),
                                k2[:, kk2] * Int.(ones(num5)'),
                                k3[:, kk3] * Int.(ones(num5)'),
                                k4[:, kk4] * Int.(ones(num5)'),
                                k5,
                                dims = 3,
                            )
                        end
            ...
            V = vcat(
                ...
                vec(
                    reshape(
                        transpose(Chui_periodic(X, m, [j[1], j[2], j[3], j[4], j[5]], k)),
                        :,
                        1,
                    ),
                ),
        ...
        if 2^j > 2 * m - 1
            ...
            for i = 1:mm
                for ll = 0:2*m-2
                    ...
                    y[:, i] =
                        y[:, i] +
                        2^(j / 2) * CWWTtools.Chui_wavelet(2^j * (x .+ l) .- k[:, i], m)
                end
            end
...
function Chui_wavelet(x::Array{Float64}, m::Int)::Array{Float64}
    ...
    for ell = 0:m
        q =
            q +
            binomial(m, ell) .*
            CWWTtools.cardinal_bspline(Array{Float64}(n .+ 1 .- ell .- m), 2 * m)  #-m, since cardinal B-Spline
    end
    q = q .* (-1) .^ n * (2^(1.0 - m))
...
function cardinal_bspline(v::Array{Float64}, order::Int)::Array{Float64}
    ...
    for i = 1:length(v)
        ...
        elseif order == 3
            if j == 1
                y =
                    ...
                    x * (1.5000000000000000e+00 + x * (5.0000000000000000e-01))
            elseif j == 2
                y =
                    7.5000000000000000e-01 +
                    x * (0.0000000000000000e+00 + x * (-1.0000000000000000e+00))
            elseif j == 3
                y =
                    1.1250000000000000e+00 +
                    x * (-1.5000000000000000e+00 + x * (5.0000000000000000e-01))
            ...
            if j == 1
                y =
                    ...
                    x * (
                        ...
                        x * (1.0000000000000000e+00 + x * (1.6666666666666666e-01))
                    )
            elseif j == 2
                y =
                    ...
                    x * (
                        ...
                        x * (-1.0000000000000000e+00 + x * (-5.0000000000000000e-01))
                    )
            elseif j == 3
                y =
                    6.6666666666666663e-01 +
        ...
        elseif order == 5
            if j == 1
                y =
                    1.6276041666666665e+00 +
                    x * (
                        2.6041666666666665e+00 +
                        x * (
                            1.5625000000000000e+00 +
                            x * (4.1666666666666663e-01 + x * (4.1666666666666664e-02))
                        )
                    )
            elseif j == 2
                y =
                    5.7291666666666663e-01 +
                    x * (
            ...
            elseif j == 3
                y =
                    5.9895833333333326e-01 +
                    x * (
                        0.0000000000000000e+00 +
                        x * (
                            -6.2500000000000000e-01 +
                            x * (0.0000000000000000e+00 + x * (2.5000000000000000e-01))
                        )
                    )
            elseif j == 4
                y =
        ...
        elseif order == 6
            if j == 1
                y =
                    2.0249999999999999e+00 +
                    x * (
                        3.3750000000000000e+00 +
                        x * (
                            2.2500000000000000e+00 +
                            x * (
                                7.5000000000000000e-01 +
                                x * (1.2500000000000000e-01 + x * (8.3333333333333332e-03))
                            )
                        )
                    )
            elseif j == 2
                ...
                y =
                    ...
                    x * (
                        ...
                        x * (
                            ...
                            x * (
                                0.0000000000000000e+00 +
                                x * (2.5000000000000006e-01 + x * (8.3333333333333329e-02))
                            )
                        )
                    )
            elseif j == 4
        ...
        elseif order == 7
            if j == 1
                y =
                    2.5531467013888887e+00 +
                    x * (
                        4.3768229166666659e+00 +
                        x * (
                            3.1263020833333335e+00 +
                            x * (
                                1.1909722222222221e+00 +
                                x * (
                                    2.5520833333333331e-01 +
                                    x *
                                    (2.9166666666666664e-02 + x * (1.3888888888888889e-03))
                                )
                            )
                        )
                    )
            elseif j == 2
                y =
                    1.7955729166666637e-01 +
                    x * (
                        -1.3197916666666669e+00 +
                        x * (
                            -2.5703125000000000e+00 +
                            x * (
                                -1.8472222222222221e+00 +
                                x * (
                        ...
                        x * (
                            ...
                            x * (
                                ...
                                x * (
                                    ...
                                    x *
                                    (1.4583333333333334e-01 + x * (2.0833333333333332e-02))
                                )
                            )
                        )
                    )
        ...
        elseif order == 8
            if j == 1
                y =
                    3.2507936507936499e+00 +
                    x * (
                        5.6888888888888891e+00 +
                        x * (
                            4.2666666666666666e+00 +
                            x * (
                                1.7777777777777775e+00 +
                                x * (
            ...
            elseif j == 2
                y =
                    -2.2063492063492082e-01 +
                    x * (
                        -2.4111111111111110e+00 +
                        x * (
                            -3.8333333333333330e+00 +
                            x * (
                                -2.7222222222222219e+00 +
                                x * (
                                    -1.0555555555555556e+00 +
                                    x * (
            ...
            elseif j == 7
                y =
                    -2.2063492063492077e-01 +
                    x * (
                        2.4111111111111114e+00 +
                        x * (
                            -3.8333333333333330e+00 +
                            x * (
                                2.7222222222222219e+00 +
        ...
        elseif order == 9
            if j == 1
                y =
                    4.1704180036272316e+00 +
                    x * (
                        7.4140764508928569e+00 +
                        x * (
                            5.7665039062499996e+00 +
                            x * (
                                2.5628906249999996e+00 +
                                x * (
            ...
            elseif j == 2
                y =
                    -8.5608956473214359e-01 +
                    x * (
                        -4.0750837053571427e+00 +
                        x * (
                            -5.7226562500000000e+00 +
                            x * (
                                -4.0023437500000014e+00 +
                                x * (
                                    -1.6328125000000000e+00 +
                                    x * (
            ...
            elseif j == 8
                y =
                    -8.5608956473214382e-01 +
                    x * (
                        4.0750837053571427e+00 +
                        x * (
                            -5.7226562500000000e+00 +
                            x * (
                                4.0023437500000005e+00 +
                                x * (
                                    -1.6328125000000000e+00 +
                                    x * (
                                        4.0937499999999993e-01 +
        ...
        elseif order == 10
            if j == 1
                y =
                    5.3822889109347445e+00 +
                    x * (
                        9.6881200396825395e+00 +
                        x * (
                            7.7504960317460307e+00 +
                            x * (
                                3.6168981481481475e+00 +
                                x * (
                                    1.0850694444444446e+00 +
                                    x * (
            ...
            elseif j == 2
                y =
                    -1.8416969797178138e+00 +
                    x * (
                        -6.5658482142857135e+00 +
                        x * (
                            -8.5034722222222232e+00 +
                            x * (
                                -5.8645833333333339e+00 +
                                x * (
                                    -2.4704861111111116e+00 +
                                    x * (
            ...
            elseif j == 3
                y =
                    5.9915123456790131e-01 +
                    x * (
                        7.5669642857142883e-01 +
                        x * (
                            1.2599206349206369e+00 +
                            x * (
                                1.7291666666666661e+00 +
                                x * (
                                    1.3263888888888888e+00 +
                                    x * (
            ...
            elseif j == 8
                y =
                    5.9915123456790087e-01 +
                    x * (
                        -7.5669642857142860e-01 +
                        x * (
                            1.2599206349206351e+00 +
                            x * (
                                -1.7291666666666665e+00 +
                                x * (
                                    1.3263888888888888e+00 +
                                    x * (
                                        -5.9375000000000000e-01 +
                ...
                y =
                    ...
                    x * (
                        ...
                        x * (
                            -8.5034722222222232e+00 +
                            x * (
                                5.8645833333333330e+00 +
                                x * (
                                    -2.4704861111111112e+00 +
                                    x * (
